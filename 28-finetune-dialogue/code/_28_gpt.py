...
class MyCausalSelfAttention(nn.Module):
    ...
    第 26 课新增：attn_dropout（对注意力权重随机清零）+ resid_dropout（输出随机清零），
    用于防止模型死记硬背训练数据（过拟合）。"""
    def __init__(self, n_embd, n_head, max_block=1024, dropout=0.0):
        super().__init__()
...
class MyBlock(nn.Module):
    ...
    def __init__(self, n_embd, n_head, bias=True, max_block=1024, dropout=0.0):
        super().__init__()
        self.ln_1 = MyLayerNorm(n_embd, bias=bias)
        self.attn = MyCausalSelfAttention(n_embd, n_head, max_block, dropout)
        self.ln_2 = MyLayerNorm(n_embd, bias=bias)
        self.mlp = MyMLP(n_embd, bias=bias, dropout=dropout)
...
class MyGPT(nn.Module):
    ...
    第 26 课新增 dropout 参数（nanoGPT 官方默认 0.2）。"""

    def __init__(self, vocab_size, block_size, n_layer, n_head, n_embd,
                 bias=False, tie_weights=True, max_block=1024, dropout=0.0):
        super().__init__()
        ...
        self.blocks = nn.ModuleList([
            MyBlock(n_embd, n_head, bias=bias, max_block=max_block, dropout=dropout)
